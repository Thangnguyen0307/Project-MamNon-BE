...
userRouter.put('/:id/status', authenticate, authorize(ROLE.ADMIN),  updateUserStatus); // Khóa/mở khóa tài khoản
userRouter.delete('/:id', authenticate, authorize(ROLE.ADMIN),  deleteUser); // Xóa tài khoản
userRouter.put('/:id/role', authenticate, authorize(ROLE.ADMIN),  updateUserRole); // Đổi role tài khoản
