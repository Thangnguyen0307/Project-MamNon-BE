...
export const uploadVideoChunkGeneric = async (req, res) => {
  ...
  // Notify only the acting user (from JWT payload) when processing finishes
  const notifyUserId = req.payload?.userId;
  const statusDoc = await finalizeIfDone(videoId, notifyUserId);
    if (!done){
